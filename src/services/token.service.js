...
const generateAuthTokens = async (user) => {
  const accesTokenExpires = Math.floor(new Date()/1000)+config.jwt.accessExpirationMinutes*60
  const userId = user._id
  const token = generateToken(userId, accesTokenExpires, tokenTypes.ACCESS , config.jwt.secret);
  const returnObj = {
    "access": {
      "token": token,
      "expires": new Date(accesTokenExpires * 1000)
    }
  }
  return returnObj
